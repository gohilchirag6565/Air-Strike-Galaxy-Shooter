...
namespace ChiragMobileAds
{
    ...
    public class SettingsWindow : EditorWindow
    {
        ...
        [MenuItem("Window/Chirag Ads/Chirag Ads", false, 60)]
        private static void Init()
        {
            SettingsWindow window = (SettingsWindow)GetWindow(typeof(SettingsWindow));
            window.titleContent = new GUIContent("Chirag Ads_v.1.0");
            ...
            for (int i = 0; i < advertiserSettings.Count; i++)
            {
                if (advertiserSettings[i].useSDK == true)
                {
                    if (advertiserSettings[i].advertiser == SupportedAdvertisers.Admob)
                    {
                        ...
                        ChiragAdmobPatch.SetAdmobAppID(advertiserSettings[i].platformSettings[(int)SupportedPlatforms.Android].appId.id,
        ...
        private void OnGUI()
        {
            ...
            {
                EditorSceneManager.OpenScene("Assets/ChiragPlugins/Ads/Example/TestAdsScene.unity");
            }
        ...
        {
            if (!File.Exists(Application.dataPath + "/GoogleMobileAds/Editor/ChiragAdmobPatch.cs"))
            {
                AssetDatabase.ImportPackage(Application.dataPath + "/ChiragPlugins/Ads/Patches/AdmobPatch.unitypackage", false);
            }
            AddPreprocessorDirective(Chirag.Common.Constants.USE_ADMOB_PATCH, false, BuildTargetGroup.Android);
            AddPreprocessorDirective(Chirag.Common.Constants.USE_ADMOB_PATCH, false, BuildTargetGroup.iOS);
        ...
        private void UpdateAdvertiserSettings()
        {
            ...
            {
                AdvertiserSettings advertiser = new AdvertiserSettings(SupportedAdvertisers.AdColony, "https://github.com/AdColony/", Chirag.Common.Constants.USE_ADCOLONY);
            ...
            {
                AdvertiserSettings advertiser = new AdvertiserSettings(SupportedAdvertisers.Admob, "https://github.com/googleads/googleads-mobile-unity/releases", Chirag.Common.Constants.USE_ADMOB);
                advertiser.platformSettings = new List<PlatformSettings>
                {
            ...
            {
                AdvertiserSettings advertiser = new AdvertiserSettings(SupportedAdvertisers.Chartboost, "https://answers.chartboost.com/en-us/articles/download", Chirag.Common.Constants.USE_CHARTBOOST);
                advertiser.platformSettings = new List<PlatformSettings>
                {
            ...
            {
                AdvertiserSettings advertiser = new AdvertiserSettings(SupportedAdvertisers.Heyzap, "https://developers.heyzap.com/docs/unity_sdk_setup_and_requirements", Chirag.Common.Constants.USE_HEYZAP);
                advertiser.platformSettings = new List<PlatformSettings>
                {
            ...
            {
                AdvertiserSettings advertiser = new AdvertiserSettings(SupportedAdvertisers.Unity, "https://assetstore.unity.com/packages/add-ons/services/unity-monetization-66123", Chirag.Common.Constants.USE_UNITYADS);
                advertiser.platformSettings = new List<PlatformSettings>
                {
            ...
            {
                AdvertiserSettings advertiser = new AdvertiserSettings(SupportedAdvertisers.Vungle, "https://publisher.vungle.com/sdk/plugins/unity", Chirag.Common.Constants.USE_VUNGLE);
                advertiser.platformSettings = new List<PlatformSettings>
                {
            ...
            {
                AdvertiserSettings advertiser = new AdvertiserSettings(SupportedAdvertisers.AppLovin, "https://dash.applovin.com/documentation/mediation/unity/getting-started/integration", Chirag.Common.Constants.USE_APPLOVIN);
                advertiser.platformSettings = new List<PlatformSettings>
                {
            ...
            {
                AdvertiserSettings advertiser = new AdvertiserSettings(SupportedAdvertisers.Facebook, "https://developers.facebook.com/docs/audience-network/setting-up/platform-steup/unity/add-sdk", Chirag.Common.Constants.USE_FACEBOOKADS);
                advertiser.platformSettings = new List<PlatformSettings>
                {
            ...
            {
                AdvertiserSettings advertiser = new AdvertiserSettings(SupportedAdvertisers.MoPub, "https://github.com/mopub/mopub-unity-sdk", Chirag.Common.Constants.USE_MOPUB);
                advertiser.platformSettings = new List<PlatformSettings>
                {
                    new PlatformSettings(SupportedPlatforms.Android,new AdvertiserId(),new AdvertiserId("Banner Placement ID"),new AdvertiserId("Interstitial Placement ID"),new AdvertiserId("Rewarded Placement ID"),true,true,true),
                    new PlatformSettings(SupportedPlatforms.iOS,new AdvertiserId(),new AdvertiserId("Banner Placement ID"),new AdvertiserId("Interstitial Placement ID"),new AdvertiserId("Rewarded Placement ID"),true,true,true)
            ...
            {
                AdvertiserSettings advertiser = new AdvertiserSettings(SupportedAdvertisers.IronSource, "https://developers.ironsrc.com/ironsource-mobile-unity/unity-plugin/", Chirag.Common.Constants.USE_IRONSOURCE);
                advertiser.platformSettings = new List<PlatformSettings>
                {
        ...
        public static void CreateAdSettings()
        {
            ...
            if (!AssetDatabase.IsValidFolder("Assets/ChiragPlugins/Ads/Resources"))
            {
                AssetDatabase.CreateFolder("Assets/ChiragPlugins/Ads", "Resources");
                AssetDatabase.Refresh();
            }

            AssetDatabase.CreateAsset(asset, "Assets/ChiragPlugins/Ads/Resources/AdSettingsData.asset");
            AssetDatabase.SaveAssets();
            AssetDatabase.Refresh();
        }
        ...
        private void GenerateFile()
        {
            ...
            File.WriteAllText(Application.dataPath + "/ChiragPlugins/Ads/AdOrderFile/AdOrder.txt", json);
            AssetDatabase.Refresh();
        }
        ...
        private void SetPreprocessorDirectives()
        {
            if (enableATT)
            {
                AddPreprocessorDirective(Chirag.Common.Constants.USE_ATT, false, BuildTargetGroup.iOS);
            }
            else
            {
                AddPreprocessorDirective(Chirag.Common.Constants.USE_ATT, true, BuildTargetGroup.iOS);
            }
                    ...
                    for (int j = 0; j < advertiserSettings[i].platformSettings.Count; j++)
                    {
                        ...
                        else
                        {
                            if (advertiserSettings[i].platformSettings[j].platform == SupportedPlatforms.Android)
                            {
                                if (advertiserSettings[i].advertiser == SupportedAdvertisers.Admob)
                                {
                                    AddPreprocessorDirective(Chirag.Common.Constants.USE_ADMOB_PATCH, true, BuildTargetGroup.Android);
                                }
                            ...
                            if (advertiserSettings[i].platformSettings[j].platform == SupportedPlatforms.iOS)
                            {
                                if (advertiserSettings[i].advertiser == SupportedAdvertisers.Admob)
                                {
                                    AddPreprocessorDirective(Chirag.Common.Constants.USE_ADMOB_PATCH, true, BuildTargetGroup.iOS);
                                }
        ...
        {
            if (AssetDatabase.IsValidFolder("Assets/ChiragPlugins/Ads/Plugins"))
            {
                AssetDatabase.DeleteAsset("Assets/ChiragPlugins/Ads/Plugins");
            }
;
            for (int i = 0; i < advertiserSettings.Count; i++)
            {
                if (advertiserSettings[i].useSDK == true)
                {
                    ...
                    //    if (!AssetDatabase.IsValidFolder("Assets/ChiragPlugins/Ads/Plugins"))
                    //    {
                    //        AssetDatabase.CreateFolder("Assets/ChiragPlugins/Ads", "Plugins");
                    //        AssetDatabase.Refresh();
                    //        AssetDatabase.CreateFolder("Assets/ChiragPlugins/Ads/Plugins", "Android");
                    //        AssetDatabase.Refresh();
                    //        AssetDatabase.CreateFolder("Assets/ChiragPlugins/Ads/Plugins/Android", "ChiragMobileAds;Manifest.plugin");
                    //        AssetDatabase.Refresh();
                    //        AssetDatabase.CreateFolder("Assets/ChiragPlugins/Ads/Plugins/Android/ChiragMobileAds;Manifest.plugin", "res");
                    //        AssetDatabase.Refresh();
                    //        AssetDatabase.CreateFolder("Assets/ChiragPlugins/Ads/Plugins/Android/ChiragMobileAds;Manifest.plugin/res", "xml");
                    //        AssetDatabase.Refresh();
                    ...
                    //                "</domain-config>\n" +
                    //            "</network-security-config>";
                    //        File.WriteAllText(Application.dataPath + "/ChiragPlugins/Ads/Plugins/Android/ChiragMobileAds;Manifest.plugin/res/xml/network_security_config.xml", securityConfig);
                    //    }

                    //    string text = "<?xml version=\"1.0\" encoding=\"utf-8\"?>\n" +
                    //    "<manifest xmlns:android = \"http://schemas.android.com/apk/res/android\"\n" +
                    //    "package=\"com.Chirag.mobileads\">\n" +
                    //    "<uses-sdk android:minSdkVersion=\"16\"/>\n" +
                    //    "<application" + facebookSecurity + ">\n" +
                    //    "</application>\n" +
                    //    "</manifest>";

                    //    File.WriteAllText(Application.dataPath + "/ChiragPlugins/Ads/Plugins/Android/ChiragMobileAds;Manifest.plugin/AndroidManifest.xml", text);

                    //    text = "target=android-16\nandroid.library = true";
                    //    File.WriteAllText(Application.dataPath + "/ChiragPlugins/Ads/Plugins/Android/ChiragMobileAds;Manifest.plugin/project.properties", text);
        ...
        {
            if (AssetDatabase.IsValidFolder("Assets/ChiragPlugins/Ads/Plugins/Android/ChiragMobileAds;Manifest.plugin"))
            {
                ((PluginImporter)PluginImporter.GetAtPath("Assets/ChiragPlugins/Ads/Plugins/Android/ChiragMobileAds;Manifest.plugin")).SetCompatibleWithAnyPlatform(false);
                ((PluginImporter)PluginImporter.GetAtPath("Assets/ChiragPlugins/Ads/Plugins/Android/ChiragMobileAds;Manifest.plugin")).SetCompatibleWithPlatform(BuildTarget.Android, false);
                AssetDatabase.Refresh();
            }
        }
    }
