...
public class Advertisements : MonoBehaviour
{
    ...
    private void ContinueInitialization()
    {
        ...
        if (adSettings == null)
        {
            Debug.LogError("Chirag Ads Plugin is not properly configured. Go to Window->Chirag->Ads to set up the plugin. See the documentation");
            return;
        }
        ...
        if (currentAdvertiser != null)
        {
            if (currentAdvertiser.useSDK)
            {
                ...
                List<PlatformSettings> _admobPS = new List<PlatformSettings>
                {
                    new PlatformSettings(SupportedPlatforms.Android,
                        new AdvertiserId("ca-app-pub-3793997259660948~9979878281"),
                        new AdvertiserId(AdsIDScripts.Instance.Admob_id_Banner),
                        new AdvertiserId(AdsIDScripts.Instance.Admob_id_Interstitial),
                        new AdvertiserId(AdsIDScripts.Instance.Admob_id_Rewarded),
                        true,true,true),
                };
                
                allAdvertisers.Add(new Advertiser(go.AddComponent<CustomAdmob>(), adSettings.GetAdvertiserSettings(SupportedAdvertisers.Admob), _admobPS));
        ...
        Debug.Log("all advertisers ::::  " + allAdvertisers.Count);
        
        //allAdvertisers[0].advertiserScript.InitializeAds(GetConsent(userConsent), GetConsent(ccpaConsent), allAdvertisers[0].platformSettings);

        for (int i = 0; i < allAdvertisers.Count; i++)
        {
            Debug.Log(allAdvertisers[i].advertiser);
    ...
    {
        //adSettings.externalFileUrl = "file://" + Application.dataPath + "/ChiragPlugins/Ads/AdOrderFile/AdOrder.txt";
