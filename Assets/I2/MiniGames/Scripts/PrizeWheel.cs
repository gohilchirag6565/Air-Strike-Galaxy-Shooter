...
namespace I2.MiniGames
{
	...
	{
		public static int checkFreeSpinInt=0;
		...
		public void StartSpinning() 
		{
			if (PlayerPrefs.GetInt ("GOLD") >= 100) {
				...
				PlayerPrefs.Save ();
				FXSound.THIS.fxSound.clip = FXSound.THIS.SpinningSound;
				StartSpinning (-1);
			} 
			else
				FXSound.THIS.fxSound.clip = FXSound.THIS.DisActiveBtn;


		}
		public void StartSpinningFreeAds()
		{
			checkFreeSpinInt = 1;
			FXSound.THIS.fxSound.clip = FXSound.THIS.ButtonClick;
            //ads
            //GameObject.FindObjectOfType<AdManagerUnity> ().ShowAd ("rewardedVideo");
            AdmobBannerController.Instance.ShowRewardBasedVideo();

        }
		public void SpinningCallBackAfterWatchVideoAds()
		{
			FXSound.THIS.fxSound.clip = FXSound.THIS.SpinningSound;
			StartSpinning (-1);
			checkFreeSpinInt = 0;
